...
lect::AnnotationsBuilder &lect::AnnotationsBuilder::extract_code_annotations(
    ...
    auto add = [&code_annotations, &mutex](std::string id, std::string title,
                                           std::string content,
...
lect::AnnotationsBuilder &lect::AnnotationsBuilder::
extract_text_annotations(const std::filesystem::path &root) noexcept(false) {
    ...
    std::mutex mutex;
    std::vector<TextAnnotation> &annotations = _annotations.text_annotations;
...
template <typename F>
void lect::AnnotationsBuilder::_extract_code_annotations_inner(const std::filesystem::path &path,
    ...
    if (is_directory(path)) {
        ...
        for (auto const &child : directory_iterator{path}) {
            futures.push_back(
                std::async(std::launch::async, [child, &add, &language, this] {
 ...
 */
template <typename F>
void lect::AnnotationsBuilder::_extract_text_annotations_inner(const std::filesystem::path &path,
                                     F &add) noexcept(false) {
    using namespace std::filesystem;
    if (is_directory(path)) {
        std::vector<std::future<void>> futures;
    ...
    if (content.size() < 1) {
        std::cout << color_red + "ERROR: " + color_reset + color_yellow
                  << canonical(path).string() + color_reset + ":" + color_blue +
                         std::to_string(line_counter) + color_reset +
                         "\n  The file doesn't follow the text "
                         "annotation format.\n"
