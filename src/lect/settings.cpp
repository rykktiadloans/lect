...
std::unique_ptr<lect::Settings>
lect::Settings::build_with_args(int argc, const char **argv) {
        ...
        if (arg == "-t") {
            ...
            if (!std::filesystem::is_directory(
                ...
                throw Exception(
                    ...
                    std::filesystem::canonical(settings->text_annotation_path)
                        .string() +
                    "` must be a directory");
            }
            text_path_set = true;
            ...
            if (argc == ptr + 1) {
                throw Exception("Language not supplied after " + color_green +
                                "'-l'" + color_reset);
            }
            std::string lang = argv[ptr + 1];
            ptr++;
            std::unordered_map<std::string, std::function<Language(void)>>
                language_map{{"c++", Language::cpp}, {"java", Language::java}};
            auto constructor = language_map.find(lang);
            if (constructor == language_map.end()) {
                throw Exception("Unrecognized language: " + color_blue + lang +
                                color_reset);
            }
            ...
            if (argc == ptr + 1) {
                throw Exception("Direction not supplied after " + color_green +
                                "'-d'" + color_reset +
            ...
            if (dir != "UD" && dir != "DU" && dir != "LR" && dir != "RL") {
                throw Exception("Unrecognized direction: " + color_blue + dir +
                                color_reset +
                                "\nCan be either `RL`, `LR`, `UD`, `DU`");
            }
            ...
            preprocessing_builder.remove_code_annotations_middle();
        } else if (arg == "-jb") {
            preprocessing_builder.add_jetbrains_flag();
        } else if (arg == "-suf") {
            if (argc == ptr + 1) {
            ...
            if (dir != "leaves" && dir != "roots") {
                throw Exception("Unrecognised lineup " + color_yellow + "-lup" +
                                color_reset + " option: " + color_blue + "'" +
                                dir + "'" + color_reset +
                                ".\nAvailable options: 'leaves', 'roots'");
            }
